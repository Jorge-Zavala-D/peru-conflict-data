# M1-02 source-integrity receipt

Verification timestamp: `2026-08-27T17:23:00.0554282Z` (UTC). The operation was read-only. PowerShell
`Get-FileHash -Algorithm SHA256` was run over the eleven source inputs in the
connected Dropbox project root, and recursive file counts were checked for every
active derived layer. No PDF, workbook, archive, or manifest file was written.
...
All eleven values match the M0.1 baseline. Their combined size is 33,354,916
bytes. The complete connected root remains 82 directories, 110 files, and
33,453,193 bytes; the difference is the archived initialization package and
directory metadata outside these source inputs.

## No derived-layer writes

Recursive file counts are zero under each of `02_extracted`, `03_parsed`,
`04_linked`, `05_database`, `06_validation`, and `07_releases`. The ten reports
and workbook retain their prior paths, sizes, timestamps, and hashes. M1-01/M1-02
created only ignored temporary HTML/JSONL receipts under the repository `.cache`
directory; those are not source data and are not a public source index.
