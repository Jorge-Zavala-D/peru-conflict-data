...
M1-01 and M1-02 are merged. M1-03A proved the acquisition dry run, M1-03B.1
established the reviewed production boundary, and the one-shot M1-03B.2 comparison
completed successfully and is spent. Schema `v0.2.0` is
...
M0/M0.1 and Milestone 1 are complete on protected `main`. The one-shot M1-03B.2
comparison ended `completed / all_ten_remote_bytes_identical`; no raw object was
staged or promoted. M1-04 owner review approved 50 conservative adjudications and
...
| Acceptance | M1-03B.1 had zero external/Dropbox effects; the one authorized M1-03B.2 comparison recorded every attempt and completed with all ten remote PDFs byte-identical; no compare-only path promoted raw bytes |
...
M2-02A production readiness is owner-approved by its separate fifteen-decision record.
M2-02B.1 launch design is also owner-approved: nine technical/design decisions are
closed in `config/benchmark/m2_02b1_launch_design_approval_v1.yaml`, while seven
operational launch decisions remain unresolved. Candidate v2 binds the preserved
reviewed candidate v1. Neither readiness nor design approval grants launch authority.
The candidate is documented in `docs/m2_02_annotation_execution_protocol.md` and
`docs/m2_02_coordinator_checklist.md`. A separate operational launch gate is required before annotation.
